...
## Last Updated
Ward 25 complete — 2026-08-29

## Current State
**Epic 06 LiDAR Point Cloud Pivot — LAZ-support lukker hullet mellem motoren og virkelige data.** Fem wards complete:
...
- **Ward 23** (Hardware Z-Buffer Hardening / 20M scale) — `status: gold`. Implementeringen ligger i repoet (`src/render/{batch-manager,adaptive-density,point-size,frustum-cull}.ts` + quad-billboards i `colored-point-pipeline.ts`) og alle 9 tests er grønne, men warden er aldrig blevet præsenteret for QA1. Den mangler kun sin godkendelses-gate og verifikation ved 20M punkter.
...
## What Comes Next
- **Luk Ward 23**: koden og de 9 grønne tests findes allerede. Mangler QA1-gaten og verifikation ved 20M punkter.
...
- **V1/V2 for Ward 25 mangler menneske**: drop en ægte USGS/Open Topography LAZ i `las-smoke.html`, og mål 5M-punkt LAZ i browseren mod ~3s-budgettet.
